...
class Root(object):
    ...
    @cherrypy.expose
    def index(self, s="supreme"):

        from focusapp.models import Person
        p = Person.objects.get(kbid=s)


        #user_module='person.{0}'.format(s)
        #m = importlib.import_module(user_module)
        #u = m.user
        ...
        affiliate_url = 'http://karatbars.com/signup.php?s={0}'.format(s)
        #kbgold_uk_url = 'http://www.karatbarsgold.co.uk/{0}'.format(p.kbguk_id)
        #kbintro_id = 'http://{0}.karatbarsintro.com'.format(p.kbintro_id)
